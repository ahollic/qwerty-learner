...
当完成一个章节训练后，系统会显示本章节所有错误单词的页面，此时您可以点击「练习错误单词」或「练习不熟悉单词」按钮进入专项训练。

**两种练习模式：**

- **错误单词练习**：针对拼写出错的单词进行循环练习，直到全部正确
- **不熟悉单词练习**：针对未跳过的单词（即未被标记为"已掌握"的单词）进行练习

**智能循环机制：**

- **第一轮**：练习所有目标单词（假设有 5 个）
- **第二轮**：只练习上一轮答错的单词（如果第一轮答对 3 个、错 2 个，第二轮只练习这 2 个）
- **持续循环**：每次只练习上一轮答错的单词，直到全部答对为止
- **自动退出**：当某一轮练习全部正确时，自动退出练习模式，恢复原始章节结果
...
新增练习直到单词正确的循环模式，开启以后，只要输入单词出现错误，就会一直循环当前单词，直到正确才进入下一个单词。要比纯粹的重复单词 3 遍，5 遍要高效很多。

---

### ⏭️ 跳过已掌握单词

在打字练习过程中，按下 `Esc` 键即可将当前单词标记为「已掌握」并跳过，进入下一个单词。被跳过的单词不会出现在「不熟悉单词练习」中，让你专注于真正需要学习的单词。

- 按下 `Esc` 标记当前单词为已掌握并跳过
- 被跳过的单词在结果页仍会正常显示
- 不熟悉单词练习会自动排除已跳过的单词
- 打字界面底部显示快捷键提示
...
在单词练习界面中，点击「AI 例句」按钮即可为当前单词自动生成一个英文例句及中文翻译，帮助你在真实语境中理解单词用法，加深记忆。

**特性：**

- 支持多个 AI 提供商（SiliconFlow / 智谱），通过环境变量 `VITE_AI_PROVIDER` 切换
- 例句生成后自动朗读（需配合 TTS 功能）
- 支持重新生成获取不同例句
- 切换单词时自动取消进行中的请求
...
```bash
# AI 提供方选择：siliconflow（默认） 或 zhipu
...
为 AI 生成的例句提供语音朗读功能，帮助你听到标准发音，练习听力和语感。

**特性：**

- 支持 SiliconFlow CosyVoice2 语音合成（默认）或浏览器原生 Web Speech API
- 通过环境变量 `VITE_TTS_PROVIDER` 切换（`siliconflow` / `browser`）
- 例句生成成功后自动朗读
- 支持手动播放 / 停止控制
...
```bash
# TTS 语音提供方：siliconflow（CosyVoice2） 或 browser（浏览器原生）
VITE_TTS_PROVIDER=siliconflow
```

> 💡 不配置 TTS API Key 时，可设置 `VITE_TTS_PROVIDER=browser` 使用浏览器内置语音合成，无需任何额外配置。
